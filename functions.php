...
if ( ! function_exists( 'custom_breadcrumb' ) ) {
	...
	function custom_breadcrumb( $wp_obj = null ){
		...
		'<ul>'.
		'<li>'.
		 	'<a href="'. home_url() .'"><span>ホーム
		 	</span></a>'.
		'</li>';

		if (is_attachment()) {
			/**
			*添付ファイルページ　（　$wp?obj : wp_Post）
			*添付ファイルpwー時では　is_single()もtrueになるので先に分岐
			*/
			...
			//カスタム投稿タイプかどうか
			if ( $post_type !== 'post' ){
				$the_tax = ""; //そのサイトに合わせて、投稿タイプごとに分岐させて明示的に指定しても良い
			...
			if ( $the_tax !== "" ) {
				...
				//最下層のターム配列から１つだけ取得
						...
						//最下層のタームを表示
						echo '<li>'.
						'<a href=">' .get_term_link( $term->term_id,$tax ).'">'.
						'<span>'.$term->name.'</span>'.'</a>'.'</li>';
					}}
				...
				　if ( $wp_obj->post_parent !== 0){
					$parent_array = array_reverse(
						get_post_ancestors ( $page_id));
					foreach( $parent_array as $parent_id ) {
						echo '<li>'.'<a href = ">'. get_permalink ($parent_id).'">'.'<span>'.get_the_title($parent_id ).'</span>'.					'</a>'.
						'</li>';
				}
				}
						...
						echo '<li><span>'. $page_title .'</span></li>';
							...
							/**
							 投句タイプアーカイブページ　( $wp_obj : wp_Post_Type )
							 */
							...
							*/
							...
							/**投稿アーカイブ　（$ wp_obj : WP_User ）
							 */
							echo '<li><span>'.$wp_obj->display_name.'の執筆記事</span></li>';
						}elseif ( is_archive() ){
							/**タームアーカイブ　（$ wp_obj : WP_User ）
							 */
							 $term_id = $wp_obj -> term_id;
							 $term_name = $wp_obj -> name;
							 $tax_name = $wp_obj -> taxonomy;
							/*ここでタクソノミーに紐づくカスタム投稿タイプを出力してもよし*/
